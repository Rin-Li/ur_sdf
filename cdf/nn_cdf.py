...
class CDF:
    def __init__(self,device) -> None:
        ...
        # panda robot
        self.panda = URRobot(device)

    def process_data(self, data):
        import torch

        def fps_torch(points, K):
            N = points.shape[0]
            if K >= N:
                return points
            dist = torch.full((N,), float('inf'), device=points.device)
            farthest_idx = torch.randint(0, N, (1,), device=points.device)
            sampled_idx = torch.zeros(K, dtype=torch.long, device=points.device)
            for i in range(K):
                sampled_idx[i] = farthest_idx
                new_dist = torch.norm(points - points[farthest_idx], dim=1)
        ...
        for k in keys:
            ...
            q_lib = torch.full((self.max_q_per_link, 6, 6), torch.inf, device=self.device)
            for i in range(1, 7):
                mask = (q_idx == i)
                if mask.sum() > self.max_q_per_link:
                    fps_q = fps_torch(q[mask], self.max_q_per_link)
                    q_lib[:, :, i - 1] = fps_q
                elif mask.sum() > 0:
                    q_lib[:mask.sum(), :, i - 1] = q[mask]
        ...
        final_data = {
            'x': torch.cat([v['x'].unsqueeze(0) for v in processed_data.values()], dim=0),
            'q': torch.cat([v['q'].unsqueeze(0) for v in processed_data.values()], dim=0),
            'k': torch.tensor(list(processed_data.keys()), device=self.device)
        }
    ...
    def decode_distance(self,q_batch,q_lib):
        # batch_q:(batch_q,6)
        # q_lib:(batch_x,self.max_q_per_link,6,6)
    ...
    def sample_q(self,batch_q = None):
        ...
        q_sampled = self.panda.theta_min + torch.rand(batch_q,6).to(self.device)*(self.panda.theta_max-self.panda.theta_min)
    ...
    def eval_nn(self,model,num_iter = 3):
        ...
        if eval_acc:
            # bp_sdf model
            bp_sdf = BPSDF(8,-1.0,1.0,self.panda,device)
