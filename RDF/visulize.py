...
def plot_3D_ur_with_gradient(pose,theta,bp_sdf,model,device):  
    ...
    sdf,ana_grad = bp_sdf.get_whole_body_sdf_batch(p,pose,theta,model,use_derivative=True,used_links = [0,1,2,3,4,5,6])
...
def generate_ur_mesh_sdf_points(max_dist =0.10):
    # represent SDF using basis functions
    import glob
    import mesh_to_sdf
    mesh_path = os.path.join(CUR_DIR, "../ur3e/voxel_128/*.stl")
    ...
    for i,mf in enumerate(mesh_files):
        ...
        mesh_dict[i]['sdf'] = sdf
    np.save(os.path.join(CUR_DIR, 'data', 'ur_mesh_sdf.npy'), mesh_dict)

def vis_ur_sdf(pose, theta,device):
    sdf_npy_path = os.path.join(CUR_DIR, 'data', 'ur_mesh_sdf.npy')
    data = np.load(sdf_npy_path, allow_pickle=True).item()
    trans = robot.get_transformations_each_link(pose,theta)
    pts = []
...
if __name__ =='__main__':
    ...
    robot = URRobot(args.device)
    bp_sdf = ur_rdf.BPSDF(args.n_func,args.domain_min,args.domain_max,robot,args.device)
    ...
    theta = torch.tensor([0, -np.pi/2, 0, np.pi/2, 0, np.pi/2, 0]).float().to(args.device).reshape(-1,7)
